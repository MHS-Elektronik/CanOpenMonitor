	...
	if (type == 't')
	{
		...
		while (pos < bufsize)
		{
			...
			if (pos < bufsize && databytecount < msg.len)
			{
				...
				msg.data[databytecount] = (uint8_t)byte_val;
				databytecount++;
			}
			pos++;
		}
...
bool can_canusbwin32::set_can_data(const Message& m, std::string& can_cmd)
{
	...
	can_cmd_str << std::hex << std::setfill('0') << std::setw(1) << (uint16_t)m.len;
...
{
	return (uint8_t)(!(reinterpret_cast<can_canusbwin32*>(fd0)->receive(m)));
}

extern "C"
uint8_t __stdcall canSend_driver(CAN_HANDLE fd0, Message const* m)
{
	return (uint8_t)reinterpret_cast<can_canusbwin32*>(fd0)->send(m);
}
...
if (num_devs > 0)
  ...
  if (ft_status == FT_OK)
    ...
    for (i = 0; i < num_devs; i++)
      {
      desc = dev_info[i].Description;
      snr = dev_info[i].SerialNumber;
      if (strstr(desc, "Tiny-CAN") || strstr(desc, "TINY-CAN"))
        continue; 
      sprintf_s(str, MAX_BUF_SIZE, "ftdi://%d/ %s (Snr:%s)", i, desc, snr);
      len = 1 + strnlen_s(str, MAX_BUF_SIZE);
      *(values + true_size) = (char *)malloc(len);
      strcpy_s(*(values + true_size), len, str);
	  true_size++;
      }  
    }
  }
if (dev_info)
  free(dev_info);
if (true_size)
  NativeCallDelegate(values, true_size);
}
